...
SELECT po.store_no                                                       AS 门店编号,
       ...
       IFNULL(pg.goods_code, '无')                                        AS 商品编号,
...
where 1 = 1
  AND psro.return_date >= '2019-12-01 00:00:00'
  AND '2019-12-15 23:59:59' >= psro.return_date;
```
...
where 1 = 1
  ...
  and pfp.gmt_payment between '2019-12-01' and '2019-12-31';
```

### 门店对账单

``` sql
select pfp.gmt_payment 支付日期, ps.store_name 门店, pfp.payment_no 支付单号, pft.transaction_no 流水号, pfp.payment_amount 支付金额
from p_finance_payment pfp
         left join p_store ps on ps.store_no = pfp.store_no
         left join p_finance_transaction pft on pft.payment_no = pfp.payment_no
where 1 = 1
